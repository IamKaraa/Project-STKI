...
<head>
    ...
    <title>Dashboard Mesin Pencari STKI</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <style>
        body { background-color: #f8f9fa; }
        .search-container { margin-top: 5vh; margin-bottom: 4vh; }
...
<div class="container">
    <div class="row justify-content-center search-container">
        <div class="col-md-8 text-center">
            <h2 class="mb-4 text-primary fw-bold">CJR</h2>
            <form action="{{ route('home') }}" method="GET" class="d-flex shadow-sm rounded">
                <input type="text" name="q" class="form-control form-control-lg border-0" 
                       placeholder="Cari dokumen regulasi ketenagakerjaan, stunting..." 
                       value="{{ $query ?? '' }}">
            ...
            @if($query)
                <div class="mt-3">
                    <a href="{{ route('home') }}" class="btn btn-outline-secondary btn-sm">Kembali ke Semua Dokumen</a>
                </div>
            @endif
        </div>
    </div>
    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            ...
            @if($query)
                <p class="text-muted mb-4">Menemukan {{ $total_hasil }} hasil relevan untuk <strong>"{{ $query }}"</strong></p>
            @else
                <h5 class="text-muted border-bottom pb-2 mb-4">Semua Dokumen Tersedia ({{ $total_hasil }} Dokumen)</h5>
            @endif
                ...
                <div class="card mb-3 border-0 shadow-sm result-card">
                    <div class="card-body">
                        ...
                        <p class="card-text text-muted mb-2">
                            {{ $item['konten_snippet'] ?? substr($item['konten'], 0, 150) . '...' }}
                        </p>
            ...
            @empty
                <div class="text-center mt-5">
                    <h5 class="text-muted">Tidak ada dokumen di dalam sistem.</h5>
                </div>
            @endforelse
